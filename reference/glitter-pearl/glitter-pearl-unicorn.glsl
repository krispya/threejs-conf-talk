// glitter-pearl-unicorn.glsl  (v2 - corrected against the editor's error log)
//
// ---------------------------------------------------------------------------
// WHAT THE ERROR LOG TOLD US
// ---------------------------------------------------------------------------
// 1. "No precision specified for (float)" on ~120 lines
//       -> The editor prepends NO precision qualifier. My previous advice to
//          leave it out was wrong. `precision highp float;` is required and is
//          now the first line. This single omission caused most of the errors;
//          every subsequent one is downstream noise.
//
// 2. "'vUv' : undeclared identifier"
//       -> There is no UV varying. Coordinates come from gl_FragCoord instead.
//
// 3. "'uTime' : undeclared identifier"
//       -> No time uniform is injected either. It is now declared explicitly
//          below. If it stays unbound it reads as 0.0 and the shader renders
//          static rather than broken.
//
// 4. "'uL' : undeclared identifier" (lines 154, 198)
//       -> My bug. v1 defined a LIGHT constant but never replaced the uL
//          references in the body. Fixed.
//
// 5. "'fragColor' : undeclared identifier"
//       -> No `out vec4` is declared, and no #version directive is prepended.
//          Without #version the compiler defaults to GLSL ES 1.00, where the
//          output is the built-in `gl_FragColor`. That is what this file uses.
//          (Unicorn requires a WebGL2 context, but a WebGL2 context still
//          accepts ES 1.00 shader source. The two are not in conflict.)
//
// ---------------------------------------------------------------------------
// BINDING UNIFORMS LATER
// ---------------------------------------------------------------------------
// uResolution and uTime are declared but guarded, so the shader compiles and
// renders whether or not the editor binds them. If Unicorn exposes them under
// different names, rename the two declarations below - nothing else refers to
// them. Every other control is a const; convert to `uniform float` one at a
// time once the layer is visibly rendering.
// ---------------------------------------------------------------------------

precision highp float;

uniform vec2  uResolution;   // may be unbound -> falls back to FALLBACK_RES
uniform float uTime;         // may be unbound -> shader renders static

// Used only when uResolution is unbound. Any plausible size works; this
// affects the apparent scale of the grain, not correctness.
const vec2 FALLBACK_RES = vec2(1200.0, 750.0);

// Light position in uv space: y spans -0.5..0.5, x scaled by aspect.
// Bind these to Unicorn's mouse tracking later if you want cursor control.
const vec2 LIGHT_CENTER = vec2(0.00, 0.00);
const vec2 LIGHT_SWING  = vec2(0.55, 0.30);   // drift amplitude, x and y


// ==================== CONTROLS ====================
// These were sliders in the standalone build; values match the tuned defaults.
...
const float uGrain  = 460.0;  // fine speckle frequency (raw, not /100)
const float uDens   = 1.00;   // fraction of cells holding a facet (0..1)
const float uScale  = 191.0;  // facet grid frequency (raw, not /100)
const float uSharp  = 80.0;  // specular exponent (raw, not /100)
const float uWhite  = 3.20;   // glint strength

// Theme: 0.0 = mineral on #0C0A06, 1.0 = pearl on #FAF5EA. Fractions crossfade.
...
void main(){
  // Guarded resolution: an unbound uniform reads as zero, which would make
  // this a divide by zero and turn every fragment into NaN (a flat fill).
  vec2  res = (uResolution.y > 1.0) ? uResolution : FALLBACK_RES;
  vec2  uv  = (gl_FragCoord.xy - 0.5 * res) / res.y;
  float t   = uTime;

  // Light position. If uTime is unbound this is simply static, not broken.
  vec2  lightPos = LIGHT_CENTER + vec2(sin(t * 0.32) * LIGHT_SWING.x,
