...
Experimental follows Math with three floating yellow chips: FSR Upscaler, Denoiser, and Klipp. Dennis Smolek's portrait accompanies FSR Upscaler and Denoiser, and Lukasz Kwas's portrait accompanies Klipp. `pnpm bake:experimental` (`scripts/bake-experimental.mjs`) joins `klipp1.mp4` followed by `kilpp2.mp4` into a 58.1-second silent loop at `experimental.mp4`. Both full recordings fit inside the portal's center square in a 1280 × 720 video. Sources default to `~/Downloads`, or set `EXPERIMENTAL_MEDIA` to their directory.

Games! follows Experimental as the final portal video before the closing screen, with Isaac Mason's portrait. `pnpm bake:games` (`scripts/bake-games.mjs`) creates the 30-second `games.mp4` loop from his boiler-room, Crashcat driving, Navcat crowd, and wizard videos, in that order. Sources and cut windows are recorded in `reference/games/README.md`. Recording borders and browser controls are cropped out, and the demos fit inside the portal's center square at 1280 × 720. Originals are cached in `reference/games`, or set `GAMES_MEDIA` to their directory.
