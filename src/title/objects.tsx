...
export function TitleObjects({
  ...
  const objects = useMemo(() => {
    const seed = mulberry32.create(0);
    return Array.from({ length: 12 }, (_, index) => ({
      angle: mulberry32.sample(seed) * Math.PI * 2,
      radius: 0.1 + mulberry32.sample(seed) * 0.65,
      phase: (index + mulberry32.sample(seed)) / 12,
      speed: 0.004 + mulberry32.sample(seed) * 0.0025,
      size: 0.6 + mulberry32.sample(seed) * 0.55,
      drift: 0.035 + mulberry32.sample(seed) * 0.035,
      spinX: (0.1 + mulberry32.sample(seed) * 0.08) * (index % 2 ? -1 : 1),
      spinY: (0.08 + mulberry32.sample(seed) * 0.06) * (index % 3 ? 1 : -1),
      spinZ: (0.05 + mulberry32.sample(seed) * 0.05) * (index % 4 < 2 ? -1 : 1),
    }));
  }, []);
