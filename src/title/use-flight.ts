 ...
 * The title view runs `step` first in its frame callback so the clock leads every dependent.
 */
  ...
  const step: FrameStep = (state, delta) => {
    ...
    scrim.value = lerp(
      ...
      cruising &&
        ...
        ? 1
        : 0,
      enteringPackages ? clamp(arrivalSpring(progress), 0, 1) : easing.cubicInOut(progress)
    );
